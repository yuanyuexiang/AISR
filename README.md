...
* **Agent 模式(Claude)**:`messages` 可带 `agent` 块,透传外部 MCP、工具白名单、
  `add_dirs`、system-prompt 等,驱动自主工具型 agent(含阻塞式 ask_user);可 `cancel`
...
> 计划中(尚未实现):`aisr chat`(交互式 REPL)。`cancel` 端点已实现
