...
`0.0.0.0` 会暴露到局域网——**token 是这层的唯一防线**,务必设强随机值;不需要外部
访问时也可只绑到 Docker 网关而非 `0.0.0.0`(见下)。
...
./bin/aisr serve --listen 0.0.0.0:7878          # 看到 listening on ... 即可
```
...
```bash
# 在另一个终端,仓库根目录:
AISR_TOKEN=$AISR_TOKEN docker compose -f docker/docker-compose.yml \
...
docker run --rm --add-host=host.docker.internal:host-gateway curlimages/curl \
  -s -H "Authorization: Bearer $AISR_TOKEN" \
